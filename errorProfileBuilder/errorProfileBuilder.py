...
class ErrorProfileBuilder:
  def main(self):
    ...
    print("Invoking {wd}/AlignQC/bam_to_alignment_error_plot.py to compute the error profile!".format(wd=os.path.dirname(os.path.realpath(__file__))))
    commandLine = "python {wd}/AlignQC/bam_to_alignment_error_plot.py --max_length {maxBases} -random  -r {ref} --input_index {index} -o {bamFileOnly}.error.pdf \
